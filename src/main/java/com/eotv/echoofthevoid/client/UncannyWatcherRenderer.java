...
public class UncannyWatcherRenderer extends HumanoidMobRenderer<UncannyWatcherEntity, PlayerModel<UncannyWatcherEntity>> {
    ...
    public UncannyWatcherRenderer(EntityRendererProvider.Context context) {
        super(context, new PlayerModel<>(context.bakeLayer(ModelLayers.PLAYER), false), 0.0F);
    }

    @Override
    public ResourceLocation getTextureLocation(UncannyWatcherEntity entity) {
        return WATCHER_TEXTURE;
    }

    @Override
    public void render(
            UncannyWatcherEntity entity,
            float entityYaw,
            float partialTicks,
            PoseStack poseStack,
    ...
    private static void applyGrandPauseJitter(UncannyWatcherEntity entity, float partialTicks, PoseStack poseStack) {
